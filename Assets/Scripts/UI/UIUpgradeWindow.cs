...
public class UIUpgradeWindow : MonoBehaviour
{
    ...
    public Color selectedColor = Color.cyan; // Color of the selected upgrade.
    ...
    // Variable to track the currently selected upgrade option
    int selectedIndex = -1;

    public List<ItemData> PickUpgradesBasedOnChance(List<ItemData> possibleUpgrades, int pick)
    {
        List<ItemData> selectedUpgrades = new List<ItemData>();
        List<float> cumulativeProbabilities = new List<float>();

        // Calculate cumulative probabilities for the given items.
        float totalProbability = 0f;
        foreach (var upgrade in possibleUpgrades)
        {
            totalProbability += upgrade.levelUpChance;
        ...
        for (int i = 0; i < pick; i++)
        {
            ...
            for (int j = 0; j < cumulativeProbabilities.Count; j++)
            {
                ...
                {
                    selectedUpgrades.Add(possibleUpgrades[j]);
                    possibleUpgrades.RemoveAt(j);
                    cumulativeProbabilities.RemoveAt(j);

                    // Recalculate cumulative probabilities for the remaining items.
                    totalProbability = 0f;
                    for (int k = 0; k < possibleUpgrades.Count; k++)
                    {
                        totalProbability += possibleUpgrades[k].levelUpChance;
                        cumulativeProbabilities[k] = totalProbability;
                    }
    ...
    public void SetUpgrades(PlayerInventory inventory, List<ItemData> possibleUpgrades, int pick = 3, string tooltip = "")
    {
        ...
        // Use the new utility method to pick upgrades based on their chance.
        List<ItemData> selectedUpgrades = PickUpgradesBasedOnChance(possibleUpgrades, pick);
        ...
        {
            if (activeOptions < pick && activeOptions < selectedUpgrades.Count)
            {
        ...
        selectedIndex = activeOptions > 0 ? 0 : -1;
    ...
    // Handle input for navigation and selection
    void HandleInput()
    {
        ...
        {
            selectedIndex = Mathf.Min(activeOptions - 1, selectedIndex + 1);
            UpdateSelectionVisual();
        }
        else if (Input.GetKeyDown(KeyCode.Space))
        {
            if (selectedIndex >= 0 && selectedIndex < activeOptions)
            {
                RectTransform selectedOption = upgradeOptions[selectedIndex];
                Button button = selectedOption.Find(buttonPath).GetComponent<Button>();
                if (button)
                {
    ...
    // Update the visual indication of the selected item
    void UpdateSelectionVisual()
    {
        for (int i = 0; i < upgradeOptions.Count; i++)
        {
            RectTransform option = upgradeOptions[i];
            if (option.gameObject.activeSelf)
            {
                Image background = option.GetComponent<Image>();
                if (background)
                {
                    background.color = i == selectedIndex ? selectedColor : Color.grey;
                }
            }
        }
    }
